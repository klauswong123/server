...
ifCreateInviteNews = async (req, res) => {
    const body = req.body
    const date = new Date()
    body.invite_year = date.getFullYear()
    body.invite_month = date.getMonth()+1
    body.invite_date = date
    await InviteNews.find({ invited_person: body.invited_person, invite_person: body.invite_person }, (err, ifinvite) => {
        if (ifinvite) {
...
findAndUpdateInviteNews = async (req, res) => {
    const body = req.body
    if (!body) {
        return res.status(400).json({
            success: false,
    ...
    await InviteNews.findOne({ invited_person: body.invited_person, invite_person: body.invite_person }, function(err, response){
         if(err){
            console.log(err);
            }
        else{
    ...
    await InviteNews.findByIdAndUpdate(id, {$set:req.body}, function(err, response){
             if(err){
                console.log(err);
                }
                res.send('修改成功');
